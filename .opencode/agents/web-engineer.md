...
- Request DTOs: use @Valid with Bean Validation annotations (@NotNull, @NotBlank, @Min, etc.)
...
- Return a consistent error response structure (timestamp, status, message, path)
...
- application.yml: database config, JPA settings, logging, any scaffolded integrations
...
- @SpringBootTest smoke test in bootstrap to verify context loads
